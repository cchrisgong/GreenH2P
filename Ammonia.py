...
ammonia_synthesis_cost = 4500    # ($/kg·h) 合成氨过程成本参数（示例值）
...
# 2.5 合成氨工艺及需求参数
# 反应：N2 + 3H2 -> 2NH3
ammonia_hydrogen_ratio = 0.18      # kg H2 per kg NH3（示例值）
target_ammonia_production = 10 * 1000  # kg/年，即10吨/年
hourly_ammonia_demand = target_ammonia_production / 8760
ammonia_electricity_ratio = 0.303   # kWh per kg NH3（示例值）
...
for t in range(time_steps):
    ...
    # 电力平衡：风电、光伏及电池放电需满足电解槽运行、电池充电、氢气充电与合成氨所需电耗
    model.addConstr(
...
# 5.4 氨储存动态及容量限制
model.addConstr(
    ammonia_storage_balance[0] == ammonia_storage_charge[0] - ammonia_storage_discharge[0],
...
for t in range(1, time_steps):
    ...
    model.addConstr(
        ammonia_storage_balance[t] <= ammonia_storage_capacity,
...
model.addConstrs((ammonia_synthesis_power[t] >= 0.2 * ammonia_synthesis_capacity for t in range(168, time_steps)),
                 name="ammonia_synthesis_power_constraint")
...
model.addConstrs((electrolyzer_power_AE[t] >= 0.1 * electrolyzer_capacity_AE for t in range(168, time_steps)),
                 name="electrolyzer_AE_power_min_constraint")

# ------------------- 6. 附加约束：AE型电解槽必须配备至少10%（此处0.5倍）储能电池 -------------------
...
if model.status == GRB.OPTIMAL:
    ...
    total_hydrogen_production = sum((electrolyzer_power_AE[t].x + electrolyzer_power_PEM[t].x) * electrolyzer_eff * 3.6 / 120 for t in range(time_steps))
...
if model.status == GRB.OPTIMAL:
    ...
    for i in range(12):
        ...
        plt.plot(electrolyzer_demand_PEM[start:end], label='Electrolyzer Demand PEM (kW)', linestyle='-', color='red')
...
if model.status == GRB.OPTIMAL:
    ...
    bar8 = plt.bar(x, -demand_ammonia, bottom=-(demand_electrolyzer_AE+demand_electrolyzer_PEM + demand_battery + demand_hydrogen), label='Ammonia Synthesis Demand', color='mediumvioletred')
    bar9 = plt.bar(x, -demand_surplus, bottom=-(demand_electrolyzer_AE+demand_electrolyzer_PEM + demand_battery + demand_hydrogen + demand_ammonia), label='Surplus', color='brown')
    ...
    for i in range(12):
        ...
        plt.bar(x_month, -demand_ammonia_month, bottom=-(demand_electrolyzer_month_AE + demand_electrolyzer_month_PEM + demand_battery_month + demand_hydrogen_month), label='Ammonia Synthesis Demand', color='mediumvioletred')
        plt.bar(x_month, -demand_surplus_month, bottom=-(demand_electrolyzer_month_AE + demand_electrolyzer_month_PEM + demand_battery_month + demand_hydrogen_month + demand_ammonia_month), label='Surplus', color='brown')
...
if model.status == GRB.OPTIMAL:
    ...
    demand_ammonia_production = np.array([
        ammonia_synthesis_power[t].x * ammonia_hydrogen_ratio
        for t in range(week_start, week_end)
    ])
    ...
    bar3 = plt.bar(x, -demand_ammonia_production, label='Hydrogen Demand', color='purple')
    bar4 = plt.bar(x, -demand_h2_charge, bottom = -demand_ammonia_production, label='Hydrogen Charge', color='red')
    ...
    for i in range():
        ...
        demand_ammonia_production_month = np.array([
        ammonia_synthesis_power[t].x * ammonia_hydrogen_ratio
        for t in range(start, end)
        ])
        ...
        plt.bar(x_month, -demand_ammonia_production_month, label='Hydrogen Demand', color='purple')
        plt.bar(x_month, -demand_h2_charge_month, -demand_ammonia_production_month, label='Hydrogen Charge', color='red')
